...
const ClassCard: React.FC<ClassCardType> = ({
  ...
  return (
    <div className='flex flex-col flex-wrap gap-y-3 lg:gap-y-4 border-[1.3px] border-gray-300 !z-20 relative px-4 py-5 lg:px-5 lg:py-5 w-full h-fit min-h-[528px] rounded-xl'>
      <div className='flex items-center justify-between'>
        <span className='text-dark text-lg lg:text-xl font-roboto font-medium'>
      ...
      {limitedData.length > 0 ? (
        limitedData.map((item, index) => (
          <div
            key={index}
            onClick={modalOpen}
            className={`flex items-center justify-between border-2 rounded-lg p-3 text-subtext${
              modalOpen && "cursor-pointer"
            }`}
          >
            <div className='flex items-center space-x-2'>
              <h3 className='text-subtext text-sm lg:text-base'>
                {item.subject}
              </h3>
