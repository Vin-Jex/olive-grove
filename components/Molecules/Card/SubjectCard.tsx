    ...
    >
      ...
      <div className='flex lg:flex-col lg:items-start min-[1580px]:items-center min-[1580px]:flex-row items-center gap-4'>
        {!assessments && (
          <>
            <Link
              className='w-fit border-primary border !text-primary hover:!text-white hover:bg-[#28a1b0] transition duration-200 ease-in-out hover:text-white rounded-md px-2 md:px-3 lg:px-4 !py-1.5 lg:py-3 text-sm md:text-base font-medium'
              href={btnLink2}
              passHref
            >
            ...
            <Button onClick={toggleModal} className='!py-1.5' size='sm'>
              View {assessments ? 'Assessment' : 'Lecture'}
            </Button>
          </>
        )}
        {assessments && (
