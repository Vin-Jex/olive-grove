...
export const Topic: FC<{
  ...
  return (
    <div
      ...
      className={`flex items-center gap-4 ${type === 'subsection' && 'px-3'} ${
        isActive || isViewed ? 'text-[#1E1E1E]' : 'text-[#1E1E1E80]'
      } cursor-pointer`}
    >
      {/* Edit Icon */}
      {user?.role === 'Teacher' && (
      ...
      {topic.title.length > 20 ? topic.title.slice(0, 20) + '...' : topic.title}
    </div>
  );
};
